...
export async function runAliyunChecks(ctx) {
 ...
 const acl = await fetchText(`${bucketRoot}/?acl`, { method: "GET" });
 if (!acl.error && success(acl.status) && acl.text.includes("<AccessControlPolicy>")) {
 findings.push({
 ...
 responseLength: acl.contentLength,
 timestamp: nowIso()
 });
 }
 ...
 method: "PUT",
 path: "/?policy",
 targetUrl: `${bucketRoot}/?policy`,
 responseLength: policyWrite.contentLength,
 timestamp: nowIso()
 });
 }
